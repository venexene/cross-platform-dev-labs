...
})({"26JPH":[function(require,module,exports,__globalThis) {
...
var _miniMapleJs = require("../src/miniMaple.js");
...
function addSomething() {
    ...
    const miniMaple = new (0, _miniMapleJs.MiniMaple)();
    try {
        const result = miniMaple.diff(polynomial, variable);
        resultDiv.textContent = `\u{420}\u{435}\u{437}\u{443}\u{43B}\u{44C}\u{442}\u{430}\u{442}: ${result}`;
    } catch (error) {
        resultDiv.textContent = `\u{41E}\u{448}\u{438}\u{431}\u{43A}\u{430}: ${error.message}`;
    }
}

},{"../src/miniMaple.js":"e2ZZ4"}],"e2ZZ4":[function(require,module,exports,__globalThis) {
// Класс для работы с отдельными членами полинома
class PolynomialTerm {
    ...
    // Создает экземпляр термина из строкового представления
    static parse(termString) {
        const cleanTerm = termString.replace(/\s+/g, '');
        // Определяем знак термина
        const sign = cleanTerm.startsWith('-') ? -1 : 1;
        const unsignedTerm = cleanTerm.replace(/^[+-]/, '');
        // Ищем переменную в термине
        const variableMatch = unsignedTerm.match(/[a-z]/);
        if (!variableMatch) return new PolynomialTerm(parseFloat(unsignedTerm) * sign, '', 0);
        ...
        let exponent = 0;
        if (exponentPart.startsWith('^')) exponent = parseInt(exponentPart.slice(1)) || 1;
        else if (exponentPart.length === 0) exponent = 1;
        return new PolynomialTerm(coefficient, variable, exponent);
    }
    // Вычисляет производную термина по заданной переменной
    differentiate(respectToVariable) {
        if (this.variable !== respectToVariable || this.exponent === 0) return new PolynomialTerm(0, '', 0);
...
// Основной класс для дифференцирования полиномов
class MiniMaple {
    constructor(){
        this.termPattern = /([+-]?\d*\.?\d*\*?[a-z]?\^?\d*)/g;
    }
    // Проверяет корректность формата полинома
    ...
    // Разбивает полином на отдельные термины
    parsePolynomial(expression) {
        const cleanExpression = expression.replace(/\s+/g, '');
        const termStrings = cleanExpression.match(this.termPattern) || [];
        return termStrings.filter((term)=>term.trim() !== '').map((term)=>PolynomialTerm.parse(term)).filter((term)=>term.coefficient !== 0);
    }
    // Вычисляет производную полинома по заданной переменной
    diff(expression, respectToVariable) {
        if (!this.isValidPolynomial(expression)) throw new Error("\u041D\u0435\u043A\u043E\u0440\u0440\u0435\u043A\u0442\u043D\u044B\u0439 \u0444\u043E\u0440\u043C\u0430\u0442 \u043F\u043E\u043B\u0438\u043D\u043E\u043C\u0430");
        if (!respectToVariable || !/^[a-z]$/.test(respectToVariable)) throw new Error("\u041D\u0435\u043A\u043E\u0440\u0440\u0435\u043A\u0442\u043D\u0430\u044F \u043F\u0435\u0440\u0435\u043C\u0435\u043D\u043D\u0430\u044F \u0434\u0438\u0444\u0444\u0435\u0440\u0435\u043D\u0446\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u044F");
        const terms = this.parsePolynomial(expression);
        const differentiatedTerms = terms.map((term)=>term.differentiate(respectToVariable));
    ...
    formatResult(terms) {
        ...
        const termStrings = nonZeroTerms.map((term, index)=>{
            ...
            // Для первого термина не добавляем знак '+'
...
},{}]},["26JPH","a0t4e"], "a0t4e", "parcelRequire94c2", {})

//# sourceMappingURL=lab1.31b563d9.js.map
